    ...
    <q-form @submit="registraParcipante" @reset="resetar" class="q-gutter-md">
      <div class="q-pa-md row justify-center">
        <div class="col-9">
          ...
          <q-checkbox
            v-model="aceite"
            label="Eu quero ser milionário e me registrar no Bolão Mega Brothers da Mega da Virada 2024!"
            color="red"
          />
        </div>
      </div>
  ...
  if (selecionado[indice] == false) {
    if (numerosSelecionados.length == 6) {
      $q.dialog({
        ...
        message:
          "Você já selecionou 6 números! Deselecione um número para poder selecionar outro.",
      });
      return;
    }
...
function registraParcipante() {
  ...
  if (numerosSelecionados.length != 6) {
    $q.dialog({
      title: "Alerta!",
      message:
        "Selecione 6 números para poder se registrar. Nem Mais Nem Menos !",
    });
    return;
  }
  axios
